...
void print_loop_timing(unsigned int observed_val, unsigned int last_timer_value, unsigned int cycles_per_tick, unsigned int max_time){
	robprintfbusy( (const unsigned char *)"Timer observed: %d\n", observed_val );
	robprintfbusy( (const unsigned char *)"Last Timer: %d\n", last_timer_value );
	...
	*timer_ctrl = ENABLE_MASK | CLKSEL_MASK | MODE_MASK;
	...
	while (count < 100) {
		...
		if(diff > max_time)
			max_time = diff;
		ticks++;
		print_loop_timing(observed_val, last_timer_value, cycles_per_tick, max_time);
